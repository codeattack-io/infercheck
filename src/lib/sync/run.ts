...
// ─── Bedrock eu-central-1 pricing + context seed ─────────────────────────────
// AWS Bedrock's ListFoundationModels API does not return pricing or context windows.
// This seed covers on-demand pricing for eu-central-1 (Frankfurt) sourced from
// https://aws.amazon.com/bedrock/pricing/ — last verified 2026-04-12.
...
// The seed is keyed by the underlying foundation model ID (without region prefix).
// Inference profiles (eu.*, global.*) strip their prefix before lookup.
const BEDROCK_PRICING: Record<string, BedrockPricingSeed> = {
...
async function fetchBedrockModels(): Promise<ModelRow[]> {
  ...
  try {
    ...
    // Helper: strip region prefix from inference profile ID to get the underlying model ID
    ...
    // Helper: derive modality from input modalities array
    ...
    // Helper: derive providerName from model ID prefix (for foundation models that lack the field)
    ...
    for (const [underlyingId, profile] of profileByModel.entries()) {
      ...
      // Derive modality from underlying foundation model if available
    ...
    // 2. Foundation models NOT covered by any inference profile (embedding, rerank, Qwen, etc.)
